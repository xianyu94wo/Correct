...
class FindNearestPoint(object):
    ...
    def __init__(self, sPLat, sPLon, mRLat, mRLon):
        ...
        :param mRLon:模式经向分辨率
        '''
        stationInfoFilePath = 'F:\\work\\2020Correct\\data\\stationInfo\\StationInfo_648.txt'
        self.listLat = list(pd.read_csv(stationInfoFilePath, encoding='utf-8', sep=',', engine='python')['lat'])
        self.listLon = list(pd.read_csv(stationInfoFilePath, encoding='utf-8', sep=',', engine='python')['lon'])
        self.sPLat = sPLat
        self.sPLon = sPLon
        self.mRLat = mRLat
        self.mRLon = mRLon
...
if __name__ == '__main__':
    ...
    sPLon = 89.3
    mRLon = 0.05
    stationInfoFilePath1 = 'F:\\work\\2020Correct\\data\\stationInfo\\StationInfo_1.txt'
    stDf = pd.read_csv(stationInfoFilePath1, encoding='utf-8', sep=',', engine='python')
    listLat1 = list(pd.read_csv(stationInfoFilePath1, encoding='utf-8', sep=',', engine='python')['lat'])
    listLon1 = list(pd.read_csv(stationInfoFilePath1, encoding='utf-8', sep=',', engine='python')['lon'])
    ob1 = FindNearestPoint(sPLat, sPLon, mRLat, mRLon)
    ob1.listLon = listLon1
    ob1.listLat = listLat1
    # aa = ob1.outputGridNum()
    # print(aa)

    oo = ob1.output_grid_num()
    print(oo)
    print(len(oo))
