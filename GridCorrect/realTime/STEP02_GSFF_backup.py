    ...
    def download_ftp_file(self, eleList):
        ...
        for eEle in eleList:
            ...
            if listtemp1 == []:
                ...
                if int(self.nowTime[-4:-2]) == 20 and int(aaa[-6:-4]) >= 16 and int(aaa[-6:-4]) < 18:
                    with open(logPath + 'GRID_SCMOC_'+ eEle + aaa[:-4] + '.log', 'w+') as logf1:
                        logf1.write('当前时刻为' + eEle +aaa + '\n')
                        logf1.write('{}时{}指导报尚未能获取'.format(self.nowTime,eEle) )
                if int(self.nowTime[-4:-2]) == 8 and int(aaa[-6:-4]) > 4 and int(aaa[-6:-4]) < 9:
                    with open(logPath + 'GRID_SCMOC_'+ eEle + aaa[:-4] + '.log', 'w+') as logf1:
