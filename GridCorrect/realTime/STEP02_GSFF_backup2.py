...
if __name__ == '__main__':
    ...
    element = ['TMAX', 'TMIN']
    print(localSaveGBasePath + nowTime[:-4])
    check_dir_mkdir(localSaveGBasePath + nowTime[:-4])
    check_dir_mkdir(localSaveTBasePath + nowTime[:-4])
    #############下载资料#########################
    ftpOb = FtpLoad(ftpHost, ftpPort, ftpUsr, ftpPwd)
    ftpConnect = ftpOb.ftpConnet
    fileList = ftpConnect.nlst(ftpFilePath)
    for eEle in element:
        cc = check_data(pathG, eEle, pathS)
    for eEle in eleList:
        listtemp1 = []
