...
def get_now_time(startTimeStr, duringDays):
    '''
    按当天日期和时间判断指导报起报时间
    ...
    getTime = startTime + datetime.timedelta(hours=1 * duringDays)
...
class GetDataFromFTP(object):
    ...
    def mkdir_new_dir(self):
        ...
        if self.nowTime[:-4] not in listTemp:
            ...
            ftpFilePath = self.ftpBasePath + nowTime[:-4] + '\\'
            localFilePath = self.localBasePath + nowTime[:-4] + '\\'
...
if __name__ == '__main__':
    ...
    timeStart = time.time()
    startTimeStr = '2020092708'
    for i in range(2):

        nowTime = get_now_time(startTimeStr, i)
        ...
        for i in aa:
            ...
            if realName[1] == 'TEM':
                ...
                if realName[2][:-5] == nowTime[:-2]:
                    numStr = '1'
                    grib2_to_txt(finalSaveGPath + '\\' + i, finalSaveTPath + '\\' + 'TMP_' + nowTime[:-2] + '.txt',
                                 numStr)
                else:
                    continue
        ###############转TXT为MICAPS4######################
