...
if __name__ == '__main__':
    ...
    listTime = get_now_time(duringDay,'2020090222')
    listOmiga = mkdir_dir_and_ormiga()
    ###############################################################
    for eEle in element:
        print(eEle)
        for omiga in listOmiga:
            print(omiga)
            omiga = float(omiga)
            pathM = 'F:\\work\\2020Correct\\data\\DA\\' + '{:.3f}'.format(omiga) + '\\'
            pathS = 'F:\\work\\2020Correct\\data\\verificationDA\\' + '{:.3f}'.format(omiga) + '\\'
            aa = get_correct_verification(pathO, pathM, pathZ, eEle, listTime)
            ...
            # np.savetxt(pathS + 'skilleDayforePoint_' + eEle + '.txt', skilleDayforePoint, fmt='%.4f')
            np.savetxt('{}skilleDayforePoint_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
                       skilleDayforePoint, fmt='%.4f')
            # np.savetxt(pathS + 'diffFceDayforePointABS_' + eEle + '.txt', diffFceDayforePoint, fmt='%.4f')
            np.savetxt('{}diffFceDayforePointABS_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
                       diffFceDayforePoint, fmt='%.4f')
            # np.savetxt(pathS + 'diffZdeDayforePointABS_' + eEle + '.txt', diffZdeDayforePoint, fmt='%.4f')
            np.savetxt('{}diffZdeDayforePointABS_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
                       diffZdeDayforePoint, fmt='%.4f')
            # np.savetxt(pathS + 'diffFceDayforePoint_' + eEle + '.txt', diffFceDayforePoint, fmt='%.4f')
            np.savetxt('{}diffFceDayforePoint_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
                       diffFceDayforePoint, fmt='%.4f')
            # np.savetxt(pathS + 'diffZdeDayforePoint_' + eEle + '.txt', diffZdeDayforePoint, fmt='%.4f')
            np.savetxt('{}diffZdeDayforePoint_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
                       diffZdeDayforePoint, fmt='%.4f')
            # np.savetxt(pathS + 'listSkilleDayforALL_' + eEle + '.txt', listSkilleDayforALL, fmt='%.4f')
            # np.savetxt('{}listSkilleDayforALL_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
            #            listSkilleDayforALL, fmt='%.4f')

            dfSkilleDayforALL.to_csv('{}listSkilleDayforALL_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
                                     float_format='%.2f')
            # np.savetxt('{}listRateForALL_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
            #            listRate, fmt='%.4f')
            dfRate.to_csv('{}listRateForALL_{}_{}days_{}.txt'.format(pathS, listTime[-1], duringDay, eEle),
                          float_format='%.2f')
