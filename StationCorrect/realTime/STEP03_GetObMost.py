...
def get_time(path1,dfSt):
    ...
    dfTMAXFinal.to_csv('E:\\work\\2020Correct\\data\\TM_ob_24h_648\\' + saveName + '_TMAX.txt', float_format='%.2f')
    dfTMINFinal.to_csv('E:\\work\\2020Correct\\data\\TM_ob_24h_648\\' + saveName + '_TMIN.txt', float_format='%.2f')
...
def get_time_gb(path1,dfSt,nowBJTStr):
    ...
    for i in range(24):
        ...
        getTime = startTime - datetime.timedelta(hours=i)
        getTimeStr = getTime.strftime('%Y%m%d%H')
        print(getTimeStr)
        dfO = pd.read_csv(path1 + getTimeStr + '.txt', sep=' ', engine='python', header=1).set_index('Station_Id_C')
    ...
    dfTMAXFinal = dfSt[['TMAX']]
    dfTMAXFinal = dfTMAXFinal[~dfTMAXFinal.index.duplicated(keep='first')]
    dfTMINFinal = dfSt[['TMIN']]
    dfTMINFinal = dfTMINFinal[~dfTMINFinal.index.duplicated(keep='first')]
    # 将结果输出为存储时间为名的txt中，第一列为站号，第二列为最高温，第三列为最低气温
    dfTMAXFinal.to_csv('E:\\work\\2020Correct\\data\\TM_ob_24h_648\\' + saveName + '_TMAX.txt', float_format='%.2f')
    dfTMINFinal.to_csv('E:\\work\\2020Correct\\data\\TM_ob_24h_648\\' + saveName + '_TMIN.txt', float_format='%.2f')
...
if __name__ == '__main__':
    ...
    path1 = 'E:\\work\\2020Correct\\data\\TEM_ob_1h_648\\' #实况逐小时资料位置

    aa = get_time(path1,stDf)
    print(aa)
    print('完成！')
    # nowBJTStr = '2020073121' #回补资料日期
    # 31为回补时次，15天为15 * 2
    # for duringDays in range(32):
